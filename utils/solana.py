...
# Client HTTP partagé et réutilisé (au lieu d'en créer un nouveau à chaque appel).
# Limite le nombre de connexions ouvertes en simultané pour ne pas saturer le serveur.
_client = None
...
async def get_sol_balance(address, helius_rpc):
    try:
        ...
        logger.error(f"get_sol_balance error: {e}")
        return 0.0
...
async def get_jupiter_quote(input_mint, output_mint, amount_lamports):
    try:
        ...
        logger.error(f"get_jupiter_quote error: {e}")
        return {}
...
async def get_wallet_txs(address, helius_rpc, limit=10):
    try:
        ...
        logger.error(f"get_wallet_txs error: {e}")
        return []
